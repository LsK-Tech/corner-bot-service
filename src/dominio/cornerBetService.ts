...
export class CornerBetService {
    ...
    async _login(page: Page, screenshotPath: string) {
        ...
        this.logger.log(
            'Aguardando 5 segundos para carregar a página de login...'
        )
        ...
        if (page.url().includes('cornerprobet.com/pt/user')) {
        ...
        if (!username || !password) {
            this.logger.error('CORNER_BET_USERNAME ou CORNER_BET_PASSWORD não definidos.')
            return false
        }
        ...
        try {
            ...
            this.logger.log('Aguardando 5 segundos após clique...')
            await new Promise(r => setTimeout(r, 5000))
